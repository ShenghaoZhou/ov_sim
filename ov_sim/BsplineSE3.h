...
namespace ov_core
{
  ...
  public:
    ...
    std::tuple<Eigen::Matrix3d, Eigen::Vector3d, Eigen::Vector3d, Eigen::Vector3d, Eigen::Vector3d, Eigen::Vector3d> get_motion(double timestamp)
    {
      ...
      if (get_acceleration(timestamp, R_GtoI, p_IinG, w_IinI, v_IinG, alpha_IinI, a_IinG))
      {
        return std::make_tuple(R_GtoI, p_IinG, w_IinI, v_IinG, alpha_IinI, a_IinG);
      }
      else
      {
        std::cout << "Failed to get motion at timestamp: " << timestamp << std::endl;
        R_GtoI.setIdentity();
        p_IinG.setZero();
        w_IinI.setZero();
        v_IinG.setZero();
